...
Login is automated in the main process with the stored account credentials. The login form itself is hidden behind a "ログイン画面へ" trigger on most eplus pages, so the app clicks that first when the fields aren't visible yet, then fills the revealed form and submits it. If Eplus requests an email code, the main process reads and fills it using the current login trigger time and the account's forwarding source. Interstitial notices ("必ずお読みください" / "同意して申込み") use verified, deterministic selectors, so the app clicks through them automatically too.

Refreshing an account's profile visits the main eplus.jp site before any member.eplus.jp page — landing on a member page cold, with no prior visit to the main domain, is what triggers eplus's Akamai-fronted gateway to reject the request outright instead of showing a normal login prompt.
...
- **Clash Verge / Clash** and **sing-box Clash API**: which real Clash proxy-group actually drives rotation is resolved automatically — you never name or pick it. The first time it's needed (saving, detecting IP, reading nodes), the main process reuses whatever group was already resolved, or asks the live controller for its groups (`GET /proxies`, filtering for entries that report a member list) and takes the first one. What you *do* name and manage is the **节点方案 / Node subset preset** — select **Read nodes** to pull the auto-resolved group's real members, check exactly the ones you want, then save them under a name of your choosing in **方案名称**. Presets are saved, keep their own node lists, and are switched with the **当前方案** dropdown — nothing is pre-checked, so an empty selection just means "no restriction, rotate every node in the group." The collapsible **Import proxy configuration** control is only for bootstrapping host/port/secret from a pasted YAML or JSON controller export; it's never required again after that, and group/node discovery works from the live controller alone. The main process also reads Clash `/configs` for `mixed-port`/`port` and explicitly launches the browser through that proxy, so changing which nodes are selected changes the browser's exit as well.
...
| A run's network lease keeps failing even though **Detect IP** looks fine | Detecting IP and acquiring a lease aren't the same check. If you're iterating with `npm run dev`, confirm you restarted it after any main-process change (see above) — `npm start` always rebuilds fresh, so this isn't a factor there. In Clash/sing-box mode, confirm the live controller is actually reachable at the configured host/port/secret so the proxy group can auto-resolve, and that **Read nodes** actually returned members. In direct mode, confirm **要求国家** matches what **Detect IP** actually reports. |
