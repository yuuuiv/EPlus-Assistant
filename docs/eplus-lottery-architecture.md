...
本项目是本地运行的 Eplus 抽选流程助手，面向拥有多个本人账号、需要重复填写相同或相近抽选条件的场景。

用户的主要操作为：

1. 首次从 Excel 导入账号资料，并可随时增删改或重新导入。
...
- 不绕过 CAPTCHA、滑块、人机检测、设备验证或访问频率限制。
...
- **本地优先**：凭据、会话状态、邮箱验证码和任务证据默认只存本机。
- **显式确认**：最终提交抽选前显示账号、演出、票档、枚数、希望顺位、付款方式和预计申请数，并要求一次确认。
- **可恢复**：每个账号任务均有持久化状态。程序意外退出后可从最近安全检查点继续，避免重复提交。
