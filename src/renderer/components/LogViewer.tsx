...
export function LogViewer(props: LogViewerProps) {
  ...
  return <section className="workspace-panel" aria-labelledby="logs-title">
    ...
    <div className="panel-card log-viewer">
      <div className="panel-head"><h2><CircleAlert size={16} />日志筛选</h2><select className="select-inline" aria-label="按级别筛选日志" value={level} onChange={(event) => setLevel(event.target.value as typeof level)}><option value="all">全部级别</option><option value="info">信息</option><option value="warn">提示</option><option value="error">错误</option></select></div>
