...
export function AccountDetail(props: AccountDetailProps) {
  ...
  async function refreshProfile(): Promise<void> {
    ...
    setRecords(nextRecords);
    setResults(nextResults);
    props.onMessage(refreshed.status === "Ok" ? "账号资料已更新。" : `账号资料更新完成：${refreshed.status}`);
  }

  return (
    <section className="panel-card account-detail" aria-label="账号详情">
      <div className="panel-head"><h2>{props.account.label || props.account.eplusEmail}</h2><div className="actions"><button type="button" className="icon-button" onClick={() => { void refreshProfile(); }}><RefreshCw size={15} />刷新账号资料</button><button type="button" className="icon-button" onClick={props.onClose}>关闭详情</button></div></div>
