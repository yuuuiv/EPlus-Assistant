...
- 各账号的中率排行榜，颜色以 50% 为中点做冷暖渐变（高于五成偏暖、低于五成偏冷），不只看排名，还能一眼看出偏离及格线的程度
- 最近的抽选动态、最多人抽的公演
...
**深度分析**页面在基础汇总之上，回答几个更偏"怎么用这个工具"的问题：

- 按公演日期分月统计的申请量与当选率趋势
- 同一场公演有多个账号参与抽选时，"至少中一个"的概率随参与账号数变化的曲线（这是观察性统计，不是受控实验——参与账号多的场次本身可能就是热门场次）
- 单账号对同一场公演重复申请几次与最终中选率的量效曲线
- 用 Wilson 置信区间重新打分的中签率排行，避免"只抽过1场且中了"排在"抽过20场稳定中一半"前面的小样本假象
- 按巡演系列 / 单场公演两个粒度统计的投入产出比（中选次数 ÷ 投入份数），已过滤掉样本过薄的系列/场次
- 热度（总抽选份数）与难度（参与账号中率）的相关性散点图，用来区分"人多但没那么难中"和真正难中的场次
