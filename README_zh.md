...
登录步骤由主进程自动填写账号和密码；如果 Eplus 返回邮箱验证码，主进程按本次登录触发时间、当前账号转发来源和邮件规则自动读取并填写。“必ずお読みください”“同意して申込み”这类过场通知使用的是已验证的确定性选择器，程序也会自动点过去。
...
- **Clash Verge / Clash** 与 **sing-box Clash API**：日常真正要用的是 **分组与节点** 区块——在 **当前分组** 里选一个代理组，点 **读取线路** 从正在运行的控制器实时拉取该分组的真实成员，再勾选要纳入轮换子集的节点。每个分组的勾选结果各自保存，之后切换 **当前分组** 会自动恢复该分组上次保存的选择，而不是清空重来。默认全部不勾选：不勾选任何节点表示"不限制，轮换该分组的全部节点"，所以只需要为你想收窄的节点打勾即可。下方可折叠的 **导入代理配置** 只用于一次性引导——粘贴一份 YAML 或 JSON 控制器配置，填入 `external-controller`、`secret` 和已知的分组名称列表，之后的操作都在 **分组与节点** 里完成。程序还会读取 Clash `/configs` 的 `mixed-port`/`port`，把浏览器显式接到该端口，避免只切了节点但浏览器仍走原出口。
...
| **检测 IP** 明明正常，运行的网络租约却总是失败 | 检测 IP 和获取网络租约不是同一项检查。如果你用 `npm run dev` 迭代，确认改动主进程代码后重启过它（见上文）——`npm start` 每次都会重新构建，不受这个影响。Clash/sing-box 模式下，确认“当前分组”是一个真实的代理组名称而不是单个节点名，并确认“读取线路”确实为它返回了成员。直连模式下，确认“要求国家”和“检测 IP”实际返回的国家一致 |
